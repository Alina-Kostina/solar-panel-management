...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  // Додаємо нову панель
  $result = $panelController->addPanel($panel_name, $installation_date, $location, $capacity, $tilt_angle, $status, $module_efficiency);

  if ($result) {
    header('Location: view_panels.php');
    exit;
  } else {
    $error = 'Помилка при додаванні панелі.';
  }
}
?>
...
<head>
  <meta charset="UTF-8">
  <title>Додати панель - Solar Panel Management System</title>
...
<section class="p-3">
  <div class="container-fluid">
    <h2 class="text-center">Додати нову панель</h2>
    ...
    <div class="row">
      <div class="w-50 m-auto mt-2 mb-5">
        <form class="text-center border border-info-subtle rounded-4 shadow-lg p-4" action="add_panel.php"
              method="POST">
          <div class="mb-3 text-start">
            <label class="form-label" for="panel_name">Назва панелі</label>
            <input class="form-control" type="text" name="panel_name" id="panel_name" required>
          </div>

          <div class="mb-3 text-start">
            <label class="form-label" for="installation_date">Дата встановлення</label>
            <input class="form-control" type="date" name="installation_date" id="installation_date" required>
          </div>
          ...
          <div class="mb-3 text-start">
            ...
            <input class="form-control" type="number" step="0.01" name="capacity" id="capacity" required>
          </div>

          <div class="mb-3 text-start">
            <label class="form-label" for="tilt_angle">Кут нахилу (°)</label>
            <input class="form-control" type="number" step="0.1" name="tilt_angle" id="tilt_angle" required>
          </div>
          ...
          <div class="mb-3 text-start">
            ...
            <input class="form-control" type="number" step="0.1" min="0" max="100" name="module_efficiency"
                   id="module_efficiency" required>
          </div>

          <button type="submit" class="btn btn-primary w-50 text-center">Додати панель</button>
        </form>
      </div>
    </div>
  </div>
